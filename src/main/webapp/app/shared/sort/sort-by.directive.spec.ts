...
@Component({
  template: `
    <table>
      <thead>
        <tr ccSort [(predicate)]="predicate" [(ascending)]="ascending" [callback]="transition.bind(this)">
          <th ccSortBy="name">ID<fa-icon [icon]="'sort'"></fa-icon></th>
        </tr>
      </thead>
    </table>
  `,
...
describe('Directive: SortByDirective', () => {
  ...
  it('should initialize predicate, order, icon when initial component predicate is _score', () => {
    ...
    // THEN
    expect(sortByDirective.ccSortBy).toEqual('name');
